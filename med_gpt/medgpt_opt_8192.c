...
void einsum_0(float * restrict L0_Q, float * restrict L0_X, float * restrict L0_WQ) {
    for (int m0 = 0; m0 < 128; m0 += 42) {
        for (int k0 = 0; k0 < 512; k0 += 56) {
            for (int d0 = 0; d0 < 512; d0 += 1) {
                for (int m0_i = 0; m0_i < ((m0 + 42 <= 128) ? 42 : (128 - m0)); m0_i++) {
                    int m = m0 + m0_i;
                    for (int k0_i = 0; k0_i < ((k0 + 56 <= 512) ? 56 : (512 - k0)); k0_i++) {
                        int k = k0 + k0_i;
                        for (int d0_i = 0; d0_i < ((d0 + 1 <= 512) ? 1 : (512 - d0)); d0_i++) {
                            int d = d0 + d0_i;
                            L0_Q[m * 512 + k * 1] += L0_X[m * 512 + d * 1] * L0_WQ[d * 512 + k * 1];
                        }
                    }
                }
            }
