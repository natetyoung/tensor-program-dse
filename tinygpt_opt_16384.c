...
void einsum_0(float * restrict L0_X, float * restrict L0_WQ, float * restrict L0_Q) {
    for (int s_m_L0_X=0; s_m_L0_X<32; s_m_L0_X++) {
        for (int s_d_L0_X=0; s_d_L0_X<128; s_d_L0_X++) {
            // LD/ST L0_X[(s_m_L0_X)*128 + (s_d_L0_X)*1]
            L0_X[(s_m_L0_X)*128 + (s_d_L0_X)*1] = L0_X[(s_m_L0_X)*128 + (s_d_L0_X)*1];
        }
    }
    for (int s_d_L0_WQ=0; s_d_L0_WQ<128; s_d_L0_WQ++) {
        // LD/ST L0_WQ[(s_d_L0_WQ)*128]
        L0_WQ[(s_d_L0_WQ)*128] = L0_WQ[(s_d_L0_WQ)*128];
    }
    // LD/ST L0_Q[0]
    L0_Q[0] = L0_Q[0];
}
    ...
    for (int t_m=0; t_m<32; t_m++) {
        ...
        for (int s_m_L0_S=0; s_m_L0_S<32; s_m_L0_S++) {
            for (int s_n_L0_S=0; s_n_L0_S<32; s_n_L0_S++) {
                // LD/ST L0_S[(t_m + s_m_L0_S)*32 + (s_n_L0_S)*1]
                L0_S[(t_m + s_m_L0_S)*32 + (s_n_L0_S)*1] = L0_S[(t_m + s_m_L0_S)*32 + (s_n_L0_S)*1];
            }
        }
    }
}
