...
void einsum_4(float * restrict L0_Y, float * restrict L0_W1, float * restrict L0_H) {
    for (int f_wrap=0; f_wrap<1024; f_wrap++) {
        int f_start = f_wrap * (1024 / 1024);
        int f_end = (f_wrap == 1024-1 ? 1024 : f_start + (1024 / 1024));
        for (int m=0; m<64; m++) {
...
void einsum_5(float * restrict L0_H, float * restrict L0_W2, float * restrict L0_Z) {
    for (int m_wrap=0; m_wrap<2; m_wrap++) {
        int m_start = m_wrap * (64 / 2);
        int m_end = (m_wrap == 2-1 ? 64 : m_start + (64 / 2));
