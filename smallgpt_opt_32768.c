...
void einsum_0(float * restrict L0_X, float * restrict L0_WQ, float * restrict L0_Q) {
    for (int k0 = 0; k0 < 256; k0 += 1) {
        for (int k0_i = 0; k0_i < ((k0 + 1 <= 256) ? 1 : (256 - k0)); k0_i++) {
            int k = k0 + k0_i;
            float acc = 0.0f;
            for (int m = 0; m < 64; m++) {
            for (int d = 0; d < 256; d++) {
...
void einsum_3(float * restrict L0_Y, float * restrict L0_O, float * restrict L0_Wo) {
    for (int k0 = 0; k0 < 256; k0 += 1) {
        for (int k0_i = 0; k0_i < ((k0 + 1 <= 256) ? 1 : (256 - k0)); k0_i++) {
            ...
            for (int d = 0; d < 256; d++) {
            for (int m = 0; m < 64; m++) {
            acc += L0_Y[m * 256 + d * 1] * L0_O[m * 256 + k * 1];
            }
            L0_Wo[k * 256 + d * 1] = acc;
            }
        }
...
void einsum_4(float * restrict L0_Y, float * restrict L0_W1, float * restrict L0_H) {
    for (int m0 = 0; m0 < 64; m0 += 32) {
        for (int f0 = 0; f0 < 1024; f0 += 1) {
            for (int m1 = 0; m1 < 32; m1 += 1) {
                for (int m1_i = 0; m1_i < ((m1 + 1 <= 64) ? 1 : (64 - m1)); m1_i++) {
                    int m = m1 + m0 + m1_i;
                    for (int f0_i = 0; f0_i < ((f0 + 1 <= 1024) ? 1 : (1024 - f0)); f0_i++) {
                        int f = f0 + f0_i;
                        float acc = 0.0f;
                        for (int d = 0; d < 256; d++) {
            ...
            for (int m1 = 0; m1 < 32; m1 += 1) {
                for (int m1_i = 0; m1_i < ((m1 + 1 <= 64) ? 1 : (64 - m1)); m1_i++) {
                    ...
                    for (int f0_i = 0; f0_i < ((f0 + 1 <= 1024) ? 1 : (1024 - f0)); f0_i++) {
                        ...
                        float acc = 0.0f;
                        for (int d = 0; d < 256; d++) {
                        acc += L0_Z[m * 256 + d * 1] * L0_W2[f * 256 + d * 1];
                        }
                        L0_H[m * 1024 + f * 1] = acc;
                    }
                }
