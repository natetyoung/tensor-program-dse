...
void einsum_4(float * restrict L0_Y, float * restrict L0_W1, float * restrict L0_H) {
    for (int m_wrap=0; m_wrap<256; m_wrap++) {
        int m_start = m_wrap * (256 / 256);
        int m_end = (m_wrap == 256-1 ? 256 : m_start + (256 / 256));
        for (int f_wrap=0; f_wrap<2; f_wrap++) {
            int f_start = f_wrap * (4096 / 2);
            int f_end = (f_wrap == 2-1 ? 4096 : f_start + (4096 / 2));
