...
void einsum_0(float * restrict L0_Q, float * restrict L0_X, float * restrict L0_WQ) {
    for (int k0 = 0; k0 < 1024; k0 += 341) {
        for (int d0 = 0; d0 < 1024; d0 += 1) {
            for (int k0_i = 0; k0_i < ((k0 + 341 <= 1024) ? 341 : (1024 - k0)); k0_i++) {
                int k = k0 + k0_i;
                for (int d0_i = 0; d0_i < ((d0 + 1 <= 1024) ? 1 : (1024 - d0)); d0_i++) {
                    int d = d0 + d0_i;
                    float acc = 0.0f;
                    for (int m = 0; m < 256; m++) {
                    acc += L0_Q[m * 1024 + k * 1] * L0_X[m * 1024 + d * 1];
                    }
                    L0_WQ[d * 1024 + k * 1] = acc;
                }
            }
        }
    }
...
void einsum_1(float * restrict L0_S, float * restrict L0_Q, float * restrict L0_K) {
    for (int k0 = 0; k0 < 1024; k0 += 341) {
        for (int n0 = 0; n0 < 256; n0 += 1) {
            for (int k1 = 0; k1 < 341; k1 += 1) {
                for (int k1_i = 0; k1_i < ((k1 + 1 <= 1024) ? 1 : (1024 - k1)); k1_i++) {
                    int k = k1 + k0 + k1_i;
                    for (int n0_i = 0; n0_i < ((n0 + 1 <= 256) ? 1 : (256 - n0)); n0_i++) {
                        int n = n0 + n0_i;
                        float acc = 0.0f;
                        for (int m = 0; m < 256; m++) {
...
void einsum_2(float * restrict L0_S, float * restrict L0_V, float * restrict L0_O) {
    for (int m0 = 0; m0 < 256; m0 += 85) {
        for (int k0 = 0; k0 < 1024; k0 += 1) {
            for (int m0_i = 0; m0_i < ((m0 + 85 <= 256) ? 85 : (256 - m0)); m0_i++) {
                int m = m0 + m0_i;
...
void einsum_3(float * restrict L0_Y, float * restrict L0_O, float * restrict L0_Wo) {
    for (int m0 = 0; m0 < 256; m0 += 85) {
        for (int k0 = 0; k0 < 1024; k0 += 1) {
            for (int m0_i = 0; m0_i < ((m0 + 85 <= 256) ? 85 : (256 - m0)); m0_i++) {
                int m = m0 + m0_i;
                for (int k0_i = 0; k0_i < ((k0 + 1 <= 1024) ? 1 : (1024 - k0)); k0_i++) {
                    ...
                    acc += L0_Y[m * 1024 + d * 1] * L0_O[m * 1024 + k * 1];
                    L0_Wo[k * 1024 + d * 1] = acc;
                    }
                }
            }
        }
        ...
        for (int f0 = 0; f0 < 4096; f0 += 1) {
            for (int m0_i = 0; m0_i < ((m0 + 85 <= 256) ? 85 : (256 - m0)); m0_i++) {
                ...
                for (int f0_i = 0; f0_i < ((f0 + 1 <= 4096) ? 1 : (4096 - f0)); f0_i++) {
                    ...
                    float acc = 0.0f;
                    for (int d = 0; d < 1024; d++) {
                    acc += L0_Z[m * 1024 + d * 1] * L0_H[m * 4096 + f * 1];
                    L0_W2[f * 1024 + d * 1] = acc;
                    }
                }
            }
        }
