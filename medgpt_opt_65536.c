...
void einsum_0(float * restrict L0_Q, float * restrict L0_X, float * restrict L0_WQ) {
    for (int k0 = 0; k0 < 512; k0 += 170) {
        for (int d0 = 0; d0 < 512; d0 += 1) {
            for (int k0_i = 0; k0_i < ((k0 + 170 <= 512) ? 170 : (512 - k0)); k0_i++) {
                int k = k0 + k0_i;
                for (int d0_i = 0; d0_i < ((d0 + 1 <= 512) ? 1 : (512 - d0)); d0_i++) {
                    int d = d0 + d0_i;
                    float acc = 0.0f;
                    for (int m = 0; m < 128; m++) {
                    acc += L0_Q[m * 512 + k * 1] * L0_X[m * 512 + d * 1];
                    }
                    L0_WQ[d * 512 + k * 1] = acc;
                }
            }
            ...
            for (int k1 = 0; k1 < 170; k1 += 1) {
                for (int k1_i = 0; k1_i < ((k1 + 1 <= 512) ? 1 : (512 - k1)); k1_i++) {
                    ...
                    for (int n0_i = 0; n0_i < ((n0 + 1 <= 128) ? 1 : (128 - n0)); n0_i++) {
                        ...
                        float acc = 0.0f;
                        for (int m = 0; m < 128; m++) {
                        acc += L0_S[m * 128 + n * 1] * L0_Q[m * 512 + k * 1];
                        }
                        L0_K[n * 512 + k * 1] = acc;
                    }
                }
...
void einsum_2(float * restrict L0_S, float * restrict L0_V, float * restrict L0_O) {
    for (int m0 = 0; m0 < 128; m0 += 42) {
        for (int k0 = 0; k0 < 512; k0 += 1) {
            for (int m0_i = 0; m0_i < ((m0 + 42 <= 128) ? 42 : (128 - m0)); m0_i++) {
                int m = m0 + m0_i;
        ...
        for (int k0 = 0; k0 < 512; k0 += 1) {
            for (int m0_i = 0; m0_i < ((m0 + 42 <= 128) ? 42 : (128 - m0)); m0_i++) {
                ...
                for (int k0_i = 0; k0_i < ((k0 + 1 <= 512) ? 1 : (512 - k0)); k0_i++) {
                    ...
                    float acc = 0.0f;
                    for (int d = 0; d < 512; d++) {
                    acc += L0_Y[m * 512 + d * 1] * L0_O[m * 512 + k * 1];
                    L0_Wo[k * 512 + d * 1] = acc;
                    }
                }
            }
